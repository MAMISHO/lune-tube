	...
	enyo.kind(
		...
		*/
		tickComponents: [
			...
			{tag: "div", style:"position: absolute;right: 0;top: 20px", components:[
				...
				{name:"fullScreen",kind: "moon.IconButton", classes:"fullscreen-button", ontap:"fullScreen", onhold: "holdScreen", components:[
		...
		*/
		create: function() {
			...
			// añadimos el evento hold
			// enyo.dispatcher.listen(document, 'hold', this.bindSafely(this.hold));
		},
		...
		*/
		...
		setQuality: function(val){
			...
			if(val !== this.quality){
				this.quality = val;
				this.$.currentResolution.setContent(this.getQuality());	
			}
		},

		qualityChanged: function(){

			this.setAvailableQualities({ "360p": "", "720p": "", "Audi": "" });
			this.$.currentResolution.setContent(this.getQuality());	
		},
		...
		*/
		tap: function(sender, e) {
			...
			if (this._hold) { // para evitar que el hold active el tap screen
		...
		holdScreen: function(inSender, e){
			console.log("hols");
			this.doBlockscreen(this);
			this._hold = true;
			return true;
		},
