	...
	*/
	enyo.kind(
		...
		*/
		components: [
			...
			{name: 'videoContainer', classes: 'moon-video-player-container', components: [
				...
				// {kind: "Audio", name: "audio", src:"", autoplay: true, onprogress: '_progress'},
		...
		*/
		...
		videoSourceChanged:function() {
			console.log("reproduciendo audio");
			console.log(this.videoSource);

			if(this.videoSource.length === 0) return;

			var source = this.videoSource[0];

			if(source.type.indexOf("audio") > -1){
				// var audio = this.videoSource.pop();
				this.$.audio.setSrc(source.src);
				console.log("es audio");
				this._sourceType = "audio";
				this.$.audio.play();
			}else{

				this.setSources(this.videoSource);
				this.load();
				// this.sources = this.videoSource;
