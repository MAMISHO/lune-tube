/*
* Funciones y componentes modificados para la compatibilidad con webOS
* @private updateSpinner: function : el código esta modificado para que no hacer
* uso de keyframes en el spinner, en su lugar se usa spinner.gif
*
* @Component moon.spinner name:"spinner", commentado y remplazadp por kind:"image"
* para restablecer los cambios, descomentar y comentar el código añadido
*/
	...
	enyo.kind(
		...
		*/
		published: {
			...
			*/
			playbackRateHash: {
				// fastForward: ['1,25', '1,5', '1,75', '2'],
				fastForward: ['6/8', '9/10', '1','5/4', '3/2', '7/4', '2'],
				// fastForward: ['5/4', '3/2', '7/4', '2'],
				rewind: ['-2', '-4', '-8', '-16'],
		...
		*/
		components: [
			...
			{name: 'videoContainer', classes: 'moon-video-player-container', components: [
				{name: 'video', kind: 'CustomVideo', classes: 'moon-video-player-video',
				...
				// {name: 'spinner', kind: "Image", src: "assets/spinner.gif", classes:"spinner-webos"}
			]},

			{kind:"Control", name: "spinnerContent"}, //classes: 'moon-video-player-spinner'}
			//* Fullscreen controls
				...
				{name: 'playerControl', classes: 'moon-video-player-bottom', showing: false, components: [
					{name: 'controls', kind: 'FittableColumns', rtl:false, classes: 'moon-video-player-controls', ontap: 'resetAutoTimeout', components: [
						...
						{name: 'controlsContainer', kind: 'Panels', arrangerKind: 'CarouselArranger', fit: true, draggable: false, classes: 'moon-video-player-controls-container', components: [
							{name: 'trickPlay', ontap:'playbackControlsTapped', components: [
								{name: 'playbackControls', classes: 'moon-video-player-control-buttons', components: [
									{name: 'jumpBack',		kind: 'moon.IconButton', small: true, onholdpulse: 'onHoldPulseBackHandler', ontap: 'onjumpBackward'},
									{name: 'rewind',		kind: 'moon.IconButton', small: true, ontap: 'rewind'},
									{name: 'fsPlayPause',	kind: 'moon.IconButton', small: true, ontap: 'playPause'},
									{name: 'fastForward',	kind: 'moon.IconButton', small: true, ontap: 'fastForward'},
									{name: 'jumpForward',	kind: 'moon.IconButton', small: true, onholdpulse: 'onHoldPulseForwardHandler', ontap: 'onjumpForward'}
								]}
							]},
						...
						{name: 'rightPremiumPlaceHolder', classes: 'moon-video-player-premium-placeholder-right', components: [
							{name: 'moreButton', kind: 'moon.IconButton', small: true, ontap: 'moreButtonTapped'}
						]}
					]},
		...
		*/
		create: function() {
			...
			/*we gonna create a spinner depents on platform*/
			if(enyo.platform.webos < 4){
				this.createComponent({
		            kind: "WebosSpinner",
		            container: this.$.spinnerContent,
		            name:"spinner"
		        });
			}else{
				this.createComponent({
		            kind: 'moon.Spinner',
		            container: this.$.spinnerContent,
		            classes: 'moon-video-player-spinner lunetube-spinner',
		            name:"spinner"
		        });
			}
		...
		speedChange: function(rate) {
			this._isPlaying = false;
			this.$.video.setPlaybackRate(this.playbackRateHash.fastForward[rate]);
		...
		sourcesChanged: function(){
			...
			if(this.getSources().length === 0) return;

			var source = this.getSources()[0];

			if(source.type.indexOf("audio") > -1){ //audio
				...
				this.$.videoContainer.destroyClientControls();
				this.createComponent({
				...
				this.createComponent({
					kind: "Poster", name: "poster", container: this.$.videoContainer
				});

				this.$.videoContainer.render();
				this.$.video.setSrc(source.src);
				this.$.video.setLoop(this.loop);
				...
				if(this.getVideo().kind === "CustomVideo"){

					this.$.video.setSourceComponents(this.sources);
					return true;
				}
		...
		*/
		hideFSControls: function(spottingHandled) {
			...
			this.bubble("onHideControlsComplete",this);
		},
		...
		*/
		playPause: function(sender, e) {
			...
			}else if(this._sourceType === "video"){*/
			...
			// }else{
/*
				console.log("Manda a reproducir ambos");
				return true;
			}*/
		},
		...
		*/
		...
		updateSpinner: function() {
			// console.log("****Acttualiza spinner***");
			var spinner = this.$.spinner;
			// console.log(this._isPlaying + "-" + !this._canPlay  + " - " + !this._errorCode);
			// console.log(this._isPlaying + "-" + this._canPlay  + " - " + this._errorCode);
		...
		/*Modificado por mamisho
			Este evento es lanzado cuando el data puede ser reproducido sin provocar lag
		*/
		canplaythrough: function(inSender, inEvent){
			this._isPlaying = true;
			this.updateSpinner();
		},

		/** 
		* Simulate start video playing meanwhile is buferring.
		*
		* @private
		*/
...
/*			this._canPlay = false;
		...
		*/
		_progress: function(sender, e) {
			/*console.log("videoPlayer.js -> _progress : estado del progreso");
			this.log(e);*/
		...
		*/
		_play: function(sender, e) {
			console.log("_play : " + this._isPlaying);
			this.sendFeedback('Play');
		},

		_isStartingPlay:false,
		
		_start: function(sender, e){
			console.log("Inicia a reproducir");
			this._isStartingPlay = true;
			this.updateSpinner();
		},
		...
		*/
		_pause: function(sender, e) {
			...
			console.log("_ispause : " + this._isPlaying);
		...
		*/
		_stop: function(sender, e) {
			...
			this.bubble("onVideoFinished",this);
		},
		...
		*/
		_setCanPlay: function(sender, e) {
			...
			console.log("->Llega el caplay : " + this.$.video.getReadyState());
			console.log(e);
			this._canPlay = true;
			this.updateSpinner();

			if(e.originator.name === "audio"){
				if(this.$.audio.getReadyState()> 2){
					console.log("inicia a reproducir audio");
					this.$.audio._play();
				}
			}else{
				if(this.$.video.getReadyState()>2){
					console.log("inicia a reproducir video");
					this.$.video._play();
				}
			}
		...
		*/
		_error: function(sender, e) {
			...
			console.log("Ocurrio un error en el player");
			console.log(this._errorCode);
		},
